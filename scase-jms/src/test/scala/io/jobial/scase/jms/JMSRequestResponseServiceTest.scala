...
class JMSRequestResponseServiceTest
  ...
  "request-response service" should "reply successfully" in {
    val serviceConfig = JMSServiceConfiguration[TestRequest[_ <: TestResponse], TestResponse](
      s"hello-test-${uuid(5)}", session.createQueue(s"hello-test-${uuid(5)}"),
      session.createQueue(s"hello-test-response-${uuid(5)}"))

    for {
      service <- serviceConfig.service(requestHandler)
      client <- serviceConfig.client[IO]
      responseConsumer <- JMSConsumer[IO, TestResponse](serviceConfig.responseDestination)
      r <- testSuccessfulStreamReply(service, client, responseConsumer)
    } yield r
  }

  "another request-response service" should "reply successfully" in {
    val serviceConfig = JMSServiceConfiguration[Req, Resp](
