...
class JMSProducer[F[_] : Concurrent, M](destination: Destination)(implicit context: JMSContext, cs: ContextShift[IO])
...
object JMSProducer {
  ...
  def apply[F[_] : Concurrent, M](destination: Destination)(implicit context: JMSContext, cs: ContextShift[IO]): JMSProducer[F, M] =
