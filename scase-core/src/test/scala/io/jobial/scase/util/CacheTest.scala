...
class CacheTest extends AsyncFlatSpec with ScaseTestHelper {
  ...
  "cache expiry" should "work" in {
    for {
      ...
      expiryA <- Deferred.tryable[IO, Unit]
      aValue <- cache.getOrCreate("a", pure("1"), { (k, v) => expiryA.complete() })
      _ = assert(aValue === "1")
      size <- cache.size
      _ = assert(size === 1)
      _ <- sleep(1.second)
      expiryB <- Deferred.tryable[IO, Unit]
      bValue <- cache.getOrCreate("b", pure("2"), { (k, v) => expiryB.complete() })
      _ = assert(bValue === "2")
      size <- cache.size
      _ = assert(size === 2)
      _ <- sleep(1.second + 1.milli)
      expiryC <- Deferred.tryable[IO, Unit]
      cValue <- cache.getOrCreate("c", pure("3"), { (k, v) => expiryC.complete() })
      _ = assert(cValue === "3")
      size <- cache.size
      _ = assert(size === 2)
      _ <- expiryA.get
