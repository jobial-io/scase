...
import io.jobial.scase.core.{ExecutionContextWithShutdown, MessageReceiveResult, TestRequest, TestRequest1}
...
class InMemoryQueueTest extends AsyncFlatSpec {
  ...
  "request-response service" should "reply" in {
    ...
    (for {
      queue <- InMemoryQueue.create[TestRequest]
      d <- Deferred[IO, TestRequest]
      _ <- queue.subscribe({ m =>
        println(m)
